...
function reverse(list: List): List {
  if (list == null || list.rest == null) {
    ...
    const new_list = reverse(list.rest);  
    return snoc(list.first, new_list);
  }
}
...
function duplicateOnes(list: List): List {
  if (list == null) {
    ...
    const new_list = duplicateOnes(list.rest);
    if (list.first == 1) {
      return cons(1, cons(1, new_list));
    } else {
      return cons(list.first, new_list);
    }
  }
}
...
function removeTrues(tree: LogicTree): LogicTree {
  switch (tree.tag) {
    case "or":
    case "and":
      return {tag: tree.tag, leftSubtree: removeTrues(tree.leftSubtree), rightSubtree: removeTrues(tree.rightSubtree)};
    case "not":
      return {tag: "not", subtree: removeTrues(tree.subtree)};
    case "bool":
      if (tree.value == true) {
        return {tag: "not", subtree: {tag: "bool", value: false}};
      } else {
        return tree;
      }
  }
